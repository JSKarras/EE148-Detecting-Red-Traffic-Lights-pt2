...
def compute_iou(box_1, box_2):
    ...
    dist_x = min(x2, x4) - max(x1, x3) + 1
    dist_y = min(y2, y4) - max(y1, y3) + 1  
    area_intersection = max(0, dist_x) * max(0, dist_y)
   
    # Margin of error, since bounding boxes are disallowed within 
    # 30 pixels of each other
    if dist_x < 30 or dist_y < 30:
        area_intersection = 0.5*(abs(x3-x4) * abs(y3-y4))
...
def get_pr(iou_thr, preds, gts):
    confidence_thrs = np.linspace(0.0, 0.5, 10)
...
for iou_thr in [0.1, 0.25, 0.5, 0.75]:
    ...
    p, r = get_pr(iou_thr, preds_train, gts_train)
    plt.plot(r, p)
    plt.scatter(r, p)
    plt.xlabel('Recall')
...
if done_tweaking:
    ...
    # Plot training set PR curves
    for iou_thr in [0.1, 0.25, 0.5, 0.75]:
        p, r = get_pr(iou_thr, preds_test, gts_test)
        print(r)
        print(p)
        plt.plot(r, p)
        plt.scatter(r, p)
